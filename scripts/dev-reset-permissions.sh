...
#
# Default behavior:
# - Resets all permissions for com.muesliplus.dev using tccutil
# - Refuses to run while MuesliPlusDev appears to be running unless forced
# - Prints the manual next steps required to re-grant permissions
#
# Usage:
#   ./scripts/dev-reset-permissions.sh
#   ./scripts/dev-reset-permissions.sh --bundle-id com.muesliplus.dev
#   ./scripts/dev-reset-permissions.sh --dry-run

BUNDLE_ID="${MUESLI_DEV_BUNDLE_ID:-com.muesliplus.dev}"
APP_PROCESS_NAME="${MUESLI_DEV_PROCESS_NAME:-MuesliPlusDev}"
APP_BUNDLE_PATH="${MUESLI_DEV_APP_PATH:-/Applications/Muesli+ Dev.app}"
DRY_RUN=0
FORCE=0
